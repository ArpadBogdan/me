...
def wordy_pyramid():
    ...
    for wordlength in range(3, 20, 2):
        url = f"https://us-central1-waldenpondpress.cloudfunctions.net/give_me_a_word?wordlength={wordlength}"
        word = (requests.get(url)).text.strip()
        pyramid.append(word)
    for wordlength in range(20, 2, -2):
        url = f"https://us-central1-waldenpondpress.cloudfunctions.net/give_me_a_word?wordlength={wordlength}"
        word = (requests.get(url)).text.strip()
        pyramid.append(word)
    return pyramid
...
def pokedex(low=1, high=5):
    ...
    for id in range(low, high, 1):
        ...
        if r.status_code is 200:
            ...
            height_check = the_json["height"]
            if height_check > height_ref:
                height_ref = height_check
                name = the_json["name"]
...
def diarist():
    ...
    file = gcode.read()
    number_of_times = file.count("M10 P1")
    gcode.close()
    number = str(number_of_times)

    mode = "w"
    with open(LOCAL + "/lasers.pew", mode, encoding="utf-8") as lasers:
        lasers.write(number)
